...
{
  UnmippedProxy() {}
  UnmippedProxy(legit::RenderGraph* renderGraph, vk::Format format, glm::uvec2 _baseSize, vk::ImageUsageFlags usageFlags, std::string debugName = "") :
    baseSize(_baseSize)
  {
    ...
    if (debugName != "")
    {
      imageProxy->SetDebugName(debugName + " img");
      imageViewProxy->SetDebugName(debugName + " view");
    }
  }
...
struct PersistentUnmippedProxy
{
  PersistentUnmippedProxy() {}
  PersistentUnmippedProxy(legit::Core* core, vk::Format format, glm::uvec2 _baseSize, vk::ImageUsageFlags usageFlags, legit::ImageUsageTypes baseUsage, std::string debugName = "") :
    baseSize(_baseSize)
  {
    auto createDesc = legit::Image::CreateInfo2d(_baseSize, 1, 1, format, usageFlags);
    this->image = std::unique_ptr<legit::Image>(new legit::Image(core->GetPhysicalDevice(), core->GetLogicalDevice(), createDesc));

    legit::ExecuteOnceQueue transferQueue(core);
    auto transferCommandBuffer = transferQueue.BeginCommandBuffer();
    {
      AddTransitionBarrier(image->GetImageData(), legit::ImageUsageTypes::Unknown, baseUsage, transferCommandBuffer);
    }
    transferQueue.EndCommandBuffer();

    this->imageView = std::unique_ptr<legit::ImageView>(new legit::ImageView(core->GetLogicalDevice(), image->GetImageData(), 0, 1, 0, 1));
    this->imageViewProxy = core->GetRenderGraph()->AddExternalImageView(imageView.get(), baseUsage);

    if (debugName != "")
    {
      core->SetDebugName(image->GetImageData(), debugName + " img");
      imageViewProxy->SetDebugName(debugName + " view");
    }
  }
...
{
  uint32_t count = 1;
  for(;baseSize.x > 1 && baseSize.y > 1; baseSize /= 2){count++;}
  return count;
}
struct MippedProxy
{
  MippedProxy(legit::RenderGraph *renderGraph, vk::Format format, glm::uvec2 _baseSize, vk::ImageUsageFlags usageFlags) :
    baseSize(_baseSize)
  {
    uint32_t mipsCount = CountMips(_baseSize);
  ...
  void BuildMips(legit::RenderGraph *renderGraph, legit::ShaderMemoryPool *memoryPool, const MippedProxy &mippedProxy, FilterTypes filterType = FilterTypes::Avg)
  {
    ...
    for (size_t mipIndex = 1; mipIndex < mippedProxy.mipImageViewProxies.size(); mipIndex++)
    {
      ...
      if (layerSize.width <= 0 || layerSize.height <= 0)
        break;
      auto srcProxyId = filterType == FilterTypes::ZeroBased ? mippedProxy.mipImageViewProxies[0]->Id() : mippedProxy.mipImageViewProxies[mipIndex - 1]->Id();
        ...
        .SetRecordFunc([this, memoryPool, srcProxyId, mipIndex, filterType](legit::RenderGraph::RenderPassContext passContext)
      {
        auto pipeineInfo = this->core->GetPipelineCache()->BindGraphicsPipeline(passContext.GetCommandBuffer(), passContext.GetRenderPass()->GetHandle(), legit::DepthSettings::Disabled(), { legit::BlendSettings::Opaque() }, legit::VertexDeclaration(), vk::PrimitiveTopology::eTriangleFan, mipLevelBuilder.shaderProgram.get());
        {
          ...
          auto dynamicUniformBindings = memoryPool->BeginSet(shaderDataSetInfo);
          {
            ...
            shaderDataBuffer->filterType = uint32_t(filterType);
            shaderDataBuffer->dstLevel = uint32_t(mipIndex);
          }
          memoryPool->EndSet();

          std::vector<legit::ImageSamplerBinding> imageSamplerBindings;
          auto srcMipView = passContext.GetImageView(srcProxyId);
          imageSamplerBindings.push_back(shaderDataSetInfo->MakeImageSamplerBinding("srcLevelSampler", srcMipView, imageSpaceSampler.get()));
