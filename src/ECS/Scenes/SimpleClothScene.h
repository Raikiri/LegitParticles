...
void CreateSimpleClothPhysicsMesh(std::vector<entt::registry> &regLayers, glm::vec2 minPoint, glm::vec2 maxPoint, glm::ivec2 baseSize)
{
  auto count = baseSize;
  ...
  for (index.y = 0; index.y < count.y; index.y++)
  {
    for (index.x = 0; index.x < count.x; index.x++)
    {
      ...
      particleEntities[index.x + index.y * count.x] = CreateParticle(reg, glm::mix(minPoint, maxPoint, glm::vec2(index) / (glm::vec2(count) - glm::vec2(1)))/* + glm::vec2(rand() % 5, rand() % 5)*/, isFixed, isFixed);
    }
  }
